...
def get_parser():
    ...
    parser.add_argument("--audio-file", type=str, default=None, help="Path to the audio file to be decoded")
...
def load_audio(filename: str) -> torch.Tensor:
    waveform, sample_rate = torchaudio.load(filename)
    return waveform, sample_rate

def extract_features(waveform: torch.Tensor, sample_rate: int, params: AttributeDict) -> torch.Tensor:
    # Transform waveform to the same sample rate as used during training
    ...
    mel_spec = transform(waveform).log()

    # Normalize
    transform = T.SlidingWindowCmn(
        cmn_window=params["cmn_window"],
        min_cmn_window=params["min_cmn_window"],
        center=True
    )
    features = transform(mel_spec)
    return features

def decode_audio_file(filename: str, params: AttributeDict, model: nn.Module, sp: spm.SentencePieceProcessor, word_table: Optional[k2.SymbolTable] = None, decoding_graph: Optional[k2.Fsa] = None, LM: Optional[LmScorer] = None, ngram_lm=None, ngram_lm_scale: float = 0.0):
    waveform, sample_rate = load_audio(filename)
    features = extract_features(waveform, sample_rate, params)
    
    batch = {
        "inputs": features.unsqueeze(0),
        "supervisions": {
            "num_frames": torch.tensor([features.size(1)], dtype=torch.long),
